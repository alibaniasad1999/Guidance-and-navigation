...
function velocity_law = VasarhelyiGuidanceLaw(position, velocity, r_agent, p_swarm)
    % position: 1 x 3N vector of the positions of all the agents
    agent_number = floor(length(position)/3);
    agent_position = reshape(position, 3, agent_number);
    agent_velocity = reshape(velocity, 3, agent_number);
    pos = agent_position;
    vel = agent_velocity;
...
%             vel_command(:, agent) = vel_command(:, agent) + p_swarm.v_ref * u_goal;
    ...
    velocity_law = reshape(vel_command, 1, 3*nb_agents);
